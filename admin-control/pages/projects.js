/* ═══════════════════════════════════════════════════════════
   ...
   STATUS OPTIONS (Capitalized style):
...
// ── Automatic Case Translation Engine ──
// Safely maps Capitalized Status values into template-compliant lowercase selector classes
(function() {
  function fixStatusBadgeClasses() {
    const badges = document.querySelectorAll('.list-item-status');
    badges.forEach(badge => {
      // Find classes like status-Ongoing, status-Completed, status-Abandoned
      badge.classList.forEach(className => {
        if (className.startsWith('status-') && /[A-Z]/.test(className)) {
          const lowerClass = className.toLowerCase();
          badge.classList.add(lowerClass);
        }
      });
    });
  }
  // Run continuous monitoring to fix matching classes as the page dynamically updates
  setInterval(fixStatusBadgeClasses, 150);
})();
