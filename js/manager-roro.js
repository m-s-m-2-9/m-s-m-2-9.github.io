/* ═══════════════════════════════════════════════════════════════════════
   js/manager-roro.js  v3.0
   RoRo — Website Intelligence Engine for MSM
   ─────────────────────────────────────────────────────────────────────
   Vanilla JS · Zero external dependencies · Self-contained IIFE

   This file is the ENGINE. Do NOT edit content here.
   Edit content in: admin-control/crazy/manager-roro.js
   ─────────────────────────────────────────────────────────────────────
   § 0  — Injected CSS
   § 1  — Defaults (fallback knowledge base)
   § 2  — Config Loader (reads window.RORO_CONFIG)
   § 3  — Response Pool Builder
...
(function () {
  ...
  const RORO_CSS = `
    ...
    .roro-header {
      ...
      background: var(--bg2); flex-shrink: 0; user-select: none; position: relative;
    }
    ...
    .roro-avatar-pulse {
      ...
      animation: roroPulse 3.5s ease-in-out infinite; pointer-events: none;
    }
    ...
    .roro-ctrl {
      ...
      display: flex; align-items: center; justify-content: center;
      font-size: 0.7rem; transition: border-color 0.2s, color 0.2s, background 0.2s;
    ...
    .roro-enc-bar {
      font-family: var(--ff-mono); font-size: 0.53rem; color: var(--text3);
      letter-spacing: 0.06em; padding: 4px 14px;
      border-bottom: 1px solid var(--border); text-align: center; flex-shrink: 0; background: var(--bg2);
    }
    ...
    .roro-chat {
      flex: 1; overflow-y: auto; overflow-x: hidden; padding: 14px 14px 6px;
    ...
    .roro-opt {
      ...
      color: var(--text2); border: 1px solid var(--border2); background: var(--bg3);
    ...
    .roro-input-row {
      ...
      padding: 10px 12px 13px; border-top: 1px solid var(--border);
      flex-shrink: 0; background: var(--bg2);
    }
    .roro-input {
      flex: 1; background: var(--bg3); border: 1px solid var(--border2); border-radius: 20px;
      color: var(--text); font-family: var(--ff-body); font-size: 0.82rem; padding: 8px 14px;
      outline: none; transition: border-color 0.25s; resize: none; line-height: 1.4;
    }
    ...
    .roro-panel--fullscreen .roro-input-row { max-width: 680px; width: 100%; margin-left: auto; margin-right: auto; }
    ...
    .roro-prompt-ticker {
      height: 26px; overflow: hidden; border-top: 1px solid var(--border);
      background: var(--bg2); flex-shrink: 0; position: relative;
      user-select: none; cursor: pointer;
    }
    .roro-prompt-track {
      display: inline-flex; align-items: center; white-space: nowrap; height: 26px;
      animation: roroTickerScroll 60s linear infinite; will-change: transform;
    }
    ...
    .roro-prompt-item {
      ...
      font-family: var(--ff-mono); font-size: 0.56rem; color: var(--text3);
      letter-spacing: 0.04em; white-space: nowrap; flex-shrink: 0; transition: color 0.18s;
    }
    ...
    .roro-action-btns {
      display: flex; flex-wrap: wrap; gap: 5px; padding: 2px 0 4px 2px;
      max-width: 84%; align-self: flex-start;
      animation: roroMsgIn 0.28s cubic-bezier(0.16,1,0.3,1);
    }
    .roro-action-btn {
      display: inline-flex; align-items: center; gap: 5px;
      font-family: var(--ff-mono); font-size: 0.59rem; letter-spacing: 0.05em;
      color: var(--text2); border: 1px solid var(--border2); background: var(--bg3);
      padding: 5px 11px; border-radius: 4px; cursor: pointer;
      text-decoration: none; white-space: nowrap; line-height: 1;
      transition: border-color 0.2s, color 0.2s, background 0.2s; outline: none;
    }
  ...
  /* ═══════════════════════════════════════════════════════════════════
     § 1 — DEFAULTS
     Fallback knowledge base used when config is missing or incomplete.
     The config loader (§ 2) overrides these with config values.
  ═══════════════════════════════════════════════════════════════════ */

  let KB = {
    owner: {
      name: 'Manomay Shailendra Misra', shortName: 'Manomay',
      born: 'August 29, 2008', birthplace: 'Andheri, Maharashtra, India',
      city: 'Bengaluru', tagline: 'Born 2008 · Mumbai · Making something of it all',
      philosophy: 'Building legacy without losing softness.',
      description: 'A seventeen-year-old creator, thinker, and builder. Nomadic by upbringing — eight cities, one consistent ambition.',
      traits: ['Ambitious','Detail-oriented','Deeply curious'],
      inspirations: ['Dieter Rams','Paul Graham','Japanese minimalism'],
      workStyle: 'Everything from scratch. No templates, no shortcuts.',
      goals: 'Build systems and stories that outlast trends.',
      contact: 'manomaysmisra2908@gmail.com',
    },
    pages: {
      home:     { label:'Home',         summary:"The entry point. Manomay's name, a tagline, and a curated navigation grid.", features:[] },
      about:    { label:'Identity',     summary:"Manomay's identity in written form.", features:[] },
      photos:   { label:'Photos',       summary:'Curated visual moments. Public albums open to all.', features:[] },
      resume:   { label:'CV / Résumé',  summary:'A clean formal résumé. Downloadable PDF at the bottom.', features:[] },
      projects: { label:'Projects',     summary:'Where the work lives — a flat list of projects with status labels.', features:[] },
      profiles: { label:'Profiles',     summary:"All of Manomay's public internet presence.", features:[] },
      journey:  { label:'Journey',      summary:'A year-by-year timeline from 2008 to the present.', features:[] },
      birthday: { label:'Clock',        summary:'A live countdown to August 29th.', features:[] },
      ...
      skills:   { label:'Traits',       summary:'Animated skill bars and a hobbies section.', features:[] },
      games:    { label:'Games',        summary:'Five built-in games.', features:[] },
      social:   { label:'Social Proof', summary:'Brands and testimonials.', features:[] },
    },
    projects: {},
    years: {},
    design: {
      summary: 'Built from scratch — pure HTML, CSS, JavaScript.',
      philosophy: 'Every element is intentional.',
      stack: ['Vanilla HTML','Vanilla CSS','Vanilla JavaScript'],
      themes: {
        dark:   { id:'dark',   label:'Noir',   desc:'Deep black. Cinematic.' },
        light:  { id:'light',  label:'Ivory',  desc:'Warm white. Editorial.' },
        slate:  { id:'slate',  label:'Slate',  desc:'Cool blue-grey. Precise.' },
        forest: { id:'forest', label:'Forest', desc:'Muted green. Calm.' },
      },
    },
    password: {
      hint: "It isn't publicly distributed. Contact page is the best route.",
      lockedSections: ['Private photo albums','Private journey entries','Full curated lists'],
    },
    features: {
      music:   "Built-in music player with two tracks.",
      themes:  "Four visual themes. Four dots top-right switch between them.",
      roro:    "I'm RoRo — the site's intelligence layer.",
      cursor:  "Custom animated cursor — dot and ring.",
      sound:   "Mechanical click sounds — toggle via the nav bar.",
      easter:  "Seven clicks on the hero name → vinyl record appears.",
      sidebar: "Desktop sidebar — hamburger icon top right.",
    },
    social: {
      instagram: { label:'Instagram', hint:'Visual work and personal moments.', url:'https://www.instagram.com/m_s_m_2_9/' },
      linkedin:  { label:'LinkedIn',  hint:'Professional profile.',             url:'https://www.linkedin.com/in/manomay-shailendra-misra' },
      x:         { label:'X',         hint:'Thoughts in real time.',            url:'https://x.com/_msm29' },
      ...
      cv:        { label:'Download CV',hint:'Full résumé.',                     url:'manomay-cv.pdf', download:true },
    },
    faq: {},
  };
  ...
  let POOL = {
    ...
    first_name_q:    ["Before we go further — what should I call you?","One thing first. What's your name?","What should I call you?"],
    name_ack:        [(n)=>`Nice to meet you, ${n}.`,(n)=>`${n}. Good.`,(n)=>`Got it — ${n}.`,(n)=>`${n}. Noted.`],
    name_followup:   ["Ask me anything about this site. I know all of it.","This place has more layers than it looks.","Where do you want to start?"],
    name_change_ack: [(o,n)=>`${n} it is. I'll use that going forward.`,(o,n)=>`Got it. ${n} from now on.`],
    return_greet:    [(n)=>`Welcome back, ${n}.`,(n)=>`${n}. You're back.`,(n)=>`Good to see you again, ${n}.`],
    return_last_page:[(p)=>`Last time you were exploring ${p}.`,(p)=>`You left off at ${p} last time.`],
    return_prompt:   ['What would you like to explore today?','Where should we go this time?','What are you looking for?'],
    nav_confirm:     [(p)=>`Taking you to ${p}.`,(p)=>`Opening ${p}.`,(p)=>`${p}. On it.`],
    theme_confirm: {
    ...
    theme_already:  [(t)=>`Already on ${t}. Nothing changed.`,(t)=>`You're already in ${t} mode.`],
    music_play:     ['Music on.','Playing background ambience.','Audio on.','Music started.'],
    music_pause:    ['Music paused.','Paused.','Audio off.','Stopped.'],
    thanks:         ['Any time.','Of course.',"That's what I'm here for.",'Always.'],
    compliment:     ['The credit goes to Manomay.',"I just run the systems. He built everything.","I'll pass that on."],
    surprise:       ['Picking somewhere you might not have been.',"Let's go off-script.",'Random destination incoming.'],
    unknown:        ["That's a bit outside my scope — I'm specifically wired to this site.","I don't have that one. My knowledge is bounded to what's here."],
    unknown_clarify:['Could you be more specific?','I want to help — can you tell me more?'],
    unknown_redir:  ['If you need a real answer, I can route you to the Contact page.',"You could always ask Manomay directly — want me to open Contact?"],
    clear_confirm:  ['Done. Everything cleared. Fresh start.',"All your data has been removed. You're anonymous again."],
    clear_prompt:   ["I can remove everything I know about you — your name, visit history, preferences. This can't be undone."],
  };
  ...
  let INTENTS = [
    ...
    { id:'nav_about',    page:'about',    kw:['about','identity','who','person','bio','biography','story','background'] },
    ...
    { id:'nav_projects', page:'projects', kw:['project','projects','national','nationals','ecommerce','iskcon','camp','work','build','built','portfolio','made'] },
  ...
  /* ═══════════════════════════════════════════════════════════════════
     § 2 — CONFIG LOADER
     Reads window.RORO_CONFIG and populates KB, POOL, SYNONYMS,
     INTENTS, and PROMPT_LIBRARY. Called once during construction.
  ═══════════════════════════════════════════════════════════════════ */
  ...
  function loadConfig() {
    ...
    if (C.social) {
      for (const [k, v] of Object.entries(C.social)) {
        KB.social[k] = v;
      }
    }
    ...
    if (C.responses) {
      ...
      /* Pure string arrays (no token replacement) */
      const pure = ['first_hello','first_intro','first_name_q','name_followup','return_prompt',
                    'music_play','music_pause','thanks','compliment','surprise',
                    'unknown','unknown_clarify','unknown_redir','clear_confirm','clear_prompt'];
      pure.forEach(k => { if (R[k]) POOL[k] = R[k]; });

      /* {n} = visitor name */
      const nameToken = ['name_ack','return_greet','name_change_ack'];
      nameToken.forEach(k => {
        if (!R[k]) return;
        POOL[k] = R[k].map(s => {
          if (typeof s === 'function') return s;
          if (s.includes('{n}')) return (n) => s.replace(/\{n\}/g, n || '');
          return s;
        });
      });

      /* {page} = page label */
      if (R.return_last_page) {
        POOL.return_last_page = R.return_last_page.map(s => {
          if (typeof s === 'function') return s;
          if (s.includes('{page}')) return (p) => s.replace(/\{page\}/g, p || '');
          return s;
        });
      }
      if (R.nav_confirm) {
        POOL.nav_confirm = R.nav_confirm.map(s => {
          if (typeof s === 'function') return s;
          if (s.includes('{page}')) return (p) => s.replace(/\{page\}/g, p || '');
          return s;
        });
      ...
      /* {theme} = theme label */
      if (R.theme_already) {
        POOL.theme_already = R.theme_already.map(s => {
          if (typeof s === 'function') return s;
          if (s.includes('{theme}')) return (t) => s.replace(/\{theme\}/g, t || '');
          return s;
        });
      }

      /* theme_confirm — nested per theme id */
      if (R.theme_confirm) {
        POOL.theme_confirm = {};
        for (const [themeId, arr] of Object.entries(R.theme_confirm)) {
          POOL.theme_confirm[themeId] = arr;
        }
      }
    }
    ...
    /* ── Synonyms ── */
    if (C.synonyms) {
      for (const [k, v] of Object.entries(C.synonyms)) {
        if (SYNONYMS[k]) {
          SYNONYMS[k] = [...new Set([...SYNONYMS[k], ...v])];
        } else {
          SYNONYMS[k] = Array.isArray(v) ? v : [v];
        }
      }
    }

    /* ── Navigation Keyword Extensions ── */
    if (C.navigationKeywords) {
      for (const [pageId, kws] of Object.entries(C.navigationKeywords)) {
        if (!Array.isArray(kws) || !kws.length) continue;
        const navIntent = INTENTS.find(i => i.id === `nav_${pageId}`);
        if (navIntent) {
          navIntent.kw = [...navIntent.kw, ...kws];
        }
      }
    }
    ...
    if (C.customIntents && Array.isArray(C.customIntents)) {
      const sorted = C.customIntents
        .filter(ci => ci.enabled !== false)
        .sort((a, b) => (b.priority || 0) - (a.priority || 0));

      sorted.forEach(ci => {
        if (!ci.id || !ci.keywords) return;
        /* Add to INTENTS array so fuzzy matching picks it up */
        INTENTS.push({
          id:      `custom_${ci.id}`,
          page:    ci.navigate || null,
          kw:      ci.keywords,
          _custom: ci, /* preserve full custom intent data */
        });
      });
    }

    /* ── Ticker ── */
    if (C.ticker && Array.isArray(C.ticker) && C.ticker.length) {
      PROMPT_LIBRARY.length = 0;
      PROMPT_LIBRARY.push(...C.ticker);
    }

    /* ── Visitor Profiles ── */
    if (C.visitorProfiles) {
      RecruiterProfiler._configSuggestions = C.visitorProfiles;
    }
  }

  /* ═══════════════════════════════════════════════════════════════════
     § 3 — RESPONSE POOL UTILITIES
  ═══════════════════════════════════════════════════════════════════ */
  ...
  function bigramSimilarity(a, b) {
    ...
    const getBigrams = s => { const r = new Set(); for (let i=0;i<s.length-1;i++) r.add(s.slice(i,i+2)); return r; };
  ...
  const MemoryEngine = {
    ...
    load() {
      try {
        ...
        if (data.lastSeen && (Date.now() - data.lastSeen) > this.EXPIRY) { this.clear(); return null; }
        return data;
      } catch { return null; }
    },
  ...
  const ToneEngine = {
    ...
    CASUAL_SIGNALS:   /\b(bruh|bro|lol|lmao|omg|ngl|tbh|fr|rn|wtf|damn|lit|fire|vibe|goat|haha|lmk|imo|btw|nope|yep|yup|yeah|cool|sick|dope|bhai|yaar)\b/i,
    ...
    detect(text) {
      ...
      if (this.RECRUITER_SIGNALS.test(text)) return 'recruiter';
      if (this.CREATIVE_SIGNALS.test(text))  return 'creative';
      if (this.FORMAL_SIGNALS.test(text))    return 'formal';
      if (this.CASUAL_SIGNALS.test(text) || this.EMOJI_SIGNAL.test(text)) return 'casual';
    ...
    adapt(msg, tone) {
      ...
      if (!msg) return msg;
      if (tone === 'casual') return msg.replace(/\bNavigating to\b/g,'Taking you to').replace(/\bI cannot\b/g,"I can't").replace(/\bThank you\b/g,'Thanks');
      if (tone === 'formal') return msg.replace(/\bOn it\.\b/g,'Navigating to the requested page.').replace(/\bGot it\.\b/g,'Understood.').replace(/\bSure thing\.\b/g,'Of course.');
      return msg;
    }
  };
  ...
  const RecruiterProfiler = {
    type: 'newcomer', confidence: 0, scores: {},
    _configSuggestions: null,
    ...
    getSuggestions() {
      if (this._configSuggestions && this._configSuggestions[this.type]) {
        return this._configSuggestions[this.type];
      }
      const map = {
  ...
  const ContextManager = {
    ...
    resolve(text) {
      if (this.hasPronoun(text) && this.entity) return `${text} [referring to: ${this.entityLabel||this.entity}]`;
      return text;
    },
  ...
  class RoRoManager {
    ...
    constructor() {
      /* Load config first — must happen before anything else */
      loadConfig();

      this._state = {
        isOpen:false,isMinimized:false,isFullscreen:false,
        hasStarted:false,awaitingName:false,awaitingRedirect:false,awaitingClear:false
      };
    ...
    _applySettings() {
      ...
      if (S.memory && S.memory.expiryDays) {
        MemoryEngine.EXPIRY = S.memory.expiryDays * 24 * 60 * 60 * 1000;
      }
    }
    ...
    _injectStyles() {
      ...
      el.id = 'roro-styles'; el.textContent = RORO_CSS;
    ...
    _injectNavButton() {
      ...
      btn.id = 'roro-btn'; btn.title = 'RoRo — Website Assistant';
    ...
    _setupIdleDetection() {
      ...
      const IDLE_MS = idleMin * 60 * 1000;
      const AWAY_MS = awayMin * 60 * 1000;
      this._idleInterval = setInterval(() => {
        if (!this._state.isOpen || this._state.isMinimized) return;
        const elapsed = Date.now() - this._lastActivity;
        if (elapsed >= AWAY_MS && this._idleState !== 'offline') this._applyActivityState('offline');
        else if (elapsed >= IDLE_MS && elapsed < AWAY_MS && this._idleState === 'active') this._applyActivityState('idle');
      }, checkMs);
    }
    ...
    _buildTicker() {
      ...
      if (!track) return;
      const C = window.RORO_CONFIG;
      const scrollSec = (C && C.settings && C.settings.ticker && C.settings.ticker.scrollSeconds) || 60;
      track.parentElement.querySelector('.roro-prompt-track') && (track.style.animationDuration = scrollSec + 's');
      ...
      track.style.animationDuration = scrollSec + 's';
    ...
    _startConversation() {
      if (this._userData && this._userData.name) {
        ...
        const lastPd  = lastPg ? KB.pages[lastPg] : null;
        this._enqueue(pick(POOL.return_greet, n));
        if (lastPd) this._enqueue(pick(POOL.return_last_page, lastPd.label));
    ...
    _handleThemeIntent(input) {
      ...
      /* Build theme map from config + defaults */
      const C = window.RORO_CONFIG;
      const configThemeKW = (C && C.themeKeywords) || {};
      const THEME_MAP = {
        dark:   ['dark','noir','black','night','midnight','dim',...(configThemeKW.dark||[])],
        light:  ['light','ivory','white','bright','day','cream','minimal','clean','brighter','lighter','whiter',...(configThemeKW.light||[])],
        slate:  ['slate','grey','gray','blue','steel','cool','muted blue',...(configThemeKW.slate||[])],
        forest: ['forest','green','nature','olive','earthy',...(configThemeKW.forest||[])],
      };
      ...
      return {
        messages:   [pick(POOL.theme_confirm[target] || ['Done.'])],
        action:     'theme',
        actionData: target,
        options:    Object.values(KB.design.themes).filter(t => t.id !== target).slice(0,3).map(t => `Switch to ${t.label}`)
      };
    }
    ...
    _handleMusicIntent(input) {
      const lower = input.toLowerCase();
      const C = window.RORO_CONFIG;
      const configMusicKW = (C && C.musicKeywords) || {};
      const playKW  = ['play music','start music','turn on music','resume music','music on','unmute',...(configMusicKW.play||[])];
      const pauseKW = ['pause music','stop music','turn off music','mute music','music off','silence','quiet','no music',...(configMusicKW.pause||[])];
    ...
    _handleSocialIntent(input) {
      ...
      if (/\b(linkedin|linked in|professional profile|connect on linkedin)\b/i.test(lower)) {
        return { messages:["LinkedIn — Manomay's professional profile."], buttons:[{label:'↗ Open LinkedIn',href:soc.linkedin?.url||'#',primary:true},{label:'Open Profiles page',href:'#',action:()=>this._go('profiles')}], options:['Show me Instagram','Download CV'] };
      }
      if (/\b(instagram|ig|insta|gram)\b/i.test(lower)) {
        return { messages:['Instagram — visual work and personal moments.'], buttons:[{label:'↗ Open Instagram',href:soc.instagram?.url||'#',primary:true},{label:'Open Profiles page',href:'#',action:()=>this._go('profiles')}], options:['Show me Photos','Open Profiles'] };
      }
      if (/\b(github|git hub)\b/i.test(lower)) {
        return { messages:['GitHub — code repositories.'], buttons:[{label:'↗ Open GitHub',href:soc.github?.url||'#',primary:true}], options:['Show me Projects','Open Profiles'] };
      }
      if (/\b(\bx\b|twitter|tweets?)\b/i.test(lower) && /\b(show|open|see|find|his|profile)\b/i.test(lower)) {
        return { messages:['X — thoughts in real time.'], buttons:[{label:'↗ Open X',href:soc.x?.url||'#',primary:true}], options:['Show me Instagram','Open Profiles'] };
      }
      if (/\b(download|get|save)\b.{0,12}\b(cv|resume|pdf|résumé)\b/i.test(lower) ||
        ...
        const cvUrl = soc.cv?.url || 'manomay-cv.pdf';
        return { messages:['Full résumé — ready to download.'], buttons:[{label:'↓ Download CV (PDF)',href:cvUrl,primary:true,download:'Manomay-Misra-CV.pdf'},{label:'Open CV page',href:'#',action:()=>this._go('resume')}], options:['See experience section','What skills are listed?'] };
      }
      if (/\b(email|send email|mail|write to|contact email|email address|contact directly)\b/i.test(lower)) {
        return { messages:['Direct line to Manomay.'], buttons:[{label:'✉ Send Email',href:'mailto:manomaysmisra2908@gmail.com',primary:true},{label:'Open Contact form',href:'#',action:()=>this._go('contact')}] };
      }
      return null;
    }
    ...
    getBotResponse(input, context) {
      ...
      /* Custom intent — high priority (checked before built-ins) */
      const highPriorityCustom = this._checkCustomIntents(input, true);
      if (highPriorityCustom) return highPriorityCustom;

      /* No intent detected */
      if (!intent) {
        const isQuestion = /\?$/.test(input.trim()) || /^(what|who|how|when|where|why|is|are|can|does|did)\b/i.test(input);
        if (isQuestion) {
          return { messages:["I'm not quite sure what you're asking. Could you be more specific?","For example: are you asking about a page, a project, or something about Manomay?"], options:['Who is Manomay?','Show me Projects','What pages are here?','Take me to Contact'] };
        }
        /* Custom intent — normal priority (after main NLP fails) */
        const normalCustom = this._checkCustomIntents(input, false);
        if (normalCustom) return normalCustom;
        return { messages:[pick(POOL.unknown), pick(POOL.unknown_redir)], awaitRedirect:true };
      }

      /* Custom intent that matched via fuzzy intent engine */
      if (intent.id.startsWith('custom_') && intent._custom) {
        return this._buildCustomResponse(intent._custom);
      }
      ...
      if (intent.id === 'info_age') {
        const born = new Date(2008,7,29); const now = new Date();
      ...
      if (intent.id === 'who_are_you') {
        return { messages:["I'm RoRo — the intelligence layer built into this site.","I know every section, can navigate for you, switch themes, control music, and answer anything about Manomay or this site.","I'm context-aware — I'll remember our conversation and adapt to how you communicate."], options:['What can you do?','Show me Projects','Who is Manomay?'] };
      }
      ...
      if (intent.id === 'info_easter') {
        return { messages:["There are a few hidden things here.", "Click Manomay's name on the homepage exactly seven times — a vinyl record appears and a hidden song plays.", "Type 'manomay' anywhere on the keyboard and the accent colour flashes.", "The splash screen can also be skipped by clicking rapidly — 20-25 clicks exits early."], options:['Take me to Home','Play music','Show me Games','Surprise me'] };
      }
      ...
      if (intent.id === 'surprise') {
        const ids = Object.keys(KB.pages); const randId = ids[Math.floor(Math.random()*ids.length)]; const pd = KB.pages[randId];
        return { messages:[pick(POOL.surprise), `Going to ${pd.label}.`, pd.summary], navigate:randId };
      }

      /* Normal-priority custom intents (after main NLP checked) */
      const normalCustom = this._checkCustomIntents(input, false);
      if (normalCustom) return normalCustom;
    ...
    _buildCustomResponse(ci) {
      ...
      return {
        messages: [msg],
        navigate: ci.navigate  || undefined,
        options:  ci.options   || undefined,
        buttons:  ci.buttons   || undefined,
      };
    }
    ...
    _pageFollowOptions(pageId) {
      const others = Object.values(KB.pages)
        ...
        .slice(0,2).map(p => `Show me ${p.label}`);
    ...
    /* ── Clear Data Detection ─────────────────────────────────── */

    _detectClearRequest(text) {
      return (
        /\b(clear|delete|forget|remove|reset|wipe|erase)\b.{0,25}\b(data|me|memory|cache|history|info|all|everything|my info)\b/i.test(text) ||
    ...
    _processQueue() {
      ...
      const C       = window.RORO_CONFIG;
      const S       = (C && C.settings && C.settings.typing) || {};
      const minDelay = S.minDelay       || 380;
      const maxDelay = S.maxDelayMs     || 2000;
      const charMult = S.charMultiplier || 12;

      const typing = this._addTypingIndicator();
      this._scrollBottom();
      const delay = Math.min(
        minDelay + Math.random() * 440 + (text ? text.length * charMult : 0),
        maxDelay
      );
    ...
    _addTypingIndicator() {
      ...
      wrap.innerHTML = `<div class="roro-bubble">
        <div class="roro-tdot"></div>
        <div class="roro-tdot"></div>
        <div class="roro-tdot"></div>
      </div>`;
      this._chatEl.appendChild(wrap);
    ...
    _type(el, text, done) {
      const C       = window.RORO_CONFIG;
      const S       = (C && C.settings && C.settings.typing) || {};
      const speedMs  = S.speedMs  || 9;
      const randomMs = S.randomMs || 18;
      ...
      const tick = () => {
        ...
        this._scrollBottom();
        setTimeout(tick, speedMs + Math.random() * randomMs);
      };
      tick();
    }
